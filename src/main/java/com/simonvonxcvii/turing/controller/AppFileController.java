...
public class AppFileController {
    ...
    public ResponseEntity<Result<UploadFileDTO>> webUploadFile(
        ...
        if (!Pattern.matches("^\\.(?i)pdf$", multipartFile.suffix()) &&
            ...
            return ResponseEntity.ok(Result.error("暂时只支持上传 .pdf、.doc、.docx、.xls 和 .xlsx 格式的文件"));
        }
        return ResponseEntity.ok(Result.ok(service.uploadFile(file, multipartFile.originalFilename(),
                multipartFile.suffix(), FileTypeEnum.getEnumByOrdinal(bizType), remark, false, false)));
    }
    ...
    public ResponseEntity<Result<UploadFileDTO>> webUploadImage(
            ...
            @Parameter(description = "文件业务类型") @RequestParam Integer bizType,
            @Parameter(description = "备注") String remark,
            @Parameter(description = "是否压缩") @RequestParam(required = false) Boolean isCompress
    ) throws IOException {
        // 检查文件
        CheckMultipartFile multipartFile = getCheckMultipartFile(file);
        // 限制可传文件格式
        if (!Pattern.matches("^\\.(?i)jpe?g$", multipartFile.suffix()) &&
            ...
            return ResponseEntity.ok(Result.error("暂时只支持上传 .jpg、.jpeg、.png 和 .webp 格式的图片"));
        }
        return ResponseEntity.ok(Result.ok(service.uploadFile(file, multipartFile.originalFilename(),
                multipartFile.suffix(), FileTypeEnum.getEnumByOrdinal(bizType), remark, isCompress, false)));
    }
    ...
    public ResponseEntity<Result<UploadFileDTO>> appUploadFile(
        ...
        return ResponseEntity.ok(Result.ok(service.uploadFile(file, multipartFile.originalFilename(), multipartFile.suffix(),
                FileTypeEnum.getEnumByOrdinal(bizType), remark, false, true)));
    }
    ...
    public ResponseEntity<Result<UploadFileDTO>> appUploadImage(
        ...
        return ResponseEntity.ok(Result.ok(service.uploadFile(file, multipartFile.originalFilename(), multipartFile.suffix(),
                FileTypeEnum.getEnumByOrdinal(bizType), remark, isCompress, true)));
    }

    @Parameter(name = "id", description = "文件 id")
    @Operation(summary = "根据文件 id 获取文件")
    @GetMapping(value = "/getFileById", produces = {MediaType.APPLICATION_PDF_VALUE, "application/msword",
            "application/vnd.ms-excel"})
    public void getFileById(@NotBlank(message = "id 不能为空") Integer id, HttpServletResponse response)
            throws IOException {
    ...
    @GetMapping(value = "/getOriginalImageById", produces = {MediaType.IMAGE_JPEG_VALUE, MediaType.IMAGE_PNG_VALUE,
            "application/webp"})
    public void getOriginalImageById(@NotBlank(message = "id 不能为空") Integer id, HttpServletResponse response)
            throws IOException {
